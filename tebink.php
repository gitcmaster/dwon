...
class App {
  ...
  public static function param($key, $default = null, $decode = false) {
    $hasPostedValue = array_key_exists($key, $_POST);
    if ($hasPostedValue) {
      $value = $_POST[$key];
    } else {
      $value = $default;
    }
    if ($decode && $hasPostedValue && $value !== null && $value !== '' && !is_array($value)) {
...
class Files {
  ...
  public static function resolve($rel) {
    ...
    if (self::isAbsolutePath($path)) {
      ...
      if ($real !== false) {
        ...
        if (!App::config('allow_outside_base') && !self::isPathWithinBase($real, $base)) {
          return false;
        }
        return $real;
      }
      ...
      if (!App::config('allow_outside_base') && (!self::isPathWithinBase($parent, $base) || !self::isPathWithinBase($path, $base))) {
        return false;
      }
    ...
    $path = ltrim($path, '/');
    if (self::hasParentTraversal($path)) {
      return false;
    }
    ...
    if ($real !== false) {
      $real = str_replace('\\', '/', $real);
      return self::isPathWithinBase($real, $base) ? $real : false;
    }
    ...
    $parent = str_replace('\\', '/', $parent);
    if (!self::isPathWithinBase($parent, $base)) {
      return false;
    }
  ...
  public static function rel($abs) {
    $base = self::base();
    $abs = str_replace('\\', '/', $abs);
    if (!self::isPathWithinBase($abs, $base)) {
  ...
  private static function hasParentTraversal($path) {
  ...
  private static function isPathWithinBase($path, $base) {
    $path = self::pathForCompare($path);
    $base = self::pathForCompare($base);
    if ($path === '' || $base === '') {
      return false;
    }
    if ($base === '/') {
    ...
    $caseInsensitive = DIRECTORY_SEPARATOR === '\\' || preg_match('/^[A-Za-z]:/', $path) || preg_match('/^[A-Za-z]:/', $base);
    if ($caseInsensitive) {
  ...
  private static function pathForCompare($path) {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    tr.batch-select-row { cursor: pointer; }
    tr.batch-select-row:hover td { background: rgba(15, 118, 110, 0.06); }
    tr.batch-select-row.is-selected td { background: rgba(15, 118, 110, 0.12); }
...
<body>
  ...
  public static function files($data) {
      ...
      <table>
        ...
        <?php else: ?>
          <?php foreach ($items as $item): ?>
            ...
            ?>
            <tr class="<?php echo App::h(trim($rowClass . ' batch-select-row')); ?>" data-selectable-row="1">
              <td>
                <input
                  type="checkbox"
                  ...
                  aria-label="Select <?php echo App::h($item['name']); ?>"
    ...
    <script>
      ...
      function setBatchDeleteRowState(checkbox) {
        if (!checkbox || !checkbox.closest) {
          return;
        }
        var row = checkbox.closest('tr[data-selectable-row="1"]');
        if (row) {
      ...
      function syncBatchDeleteSelectAll() {
        ...
        for (var i = 0; i < checkboxes.length; i++) {
          setBatchDeleteRowState(checkboxes[i]);
        }
        ...
        selectAllChk.checked = checkboxes.length > 0 && checkedCount === checkboxes.length;
        selectAllChk.indeterminate = checkedCount > 0 && checkedCount < checkboxes.length;
      }
      function toggleAllCheckboxes(btnOrCheckbox) {
        ...
        for (var i = 0; i < checkboxes.length; i++) {
          if (isButton || btnOrCheckbox === selectAllChk) {
      ...
      document.addEventListener('click', function (event) {
        ...
        var row = target.closest('tr[data-selectable-row="1"]');
        if (!row || target.closest('a, button, input, select, textarea, label, form')) {
          return;
        }
        var checkbox = row.querySelector('input[type="checkbox"][name="selected[]"][form="batch-delete-form"]');
        if (!checkbox || checkbox.disabled) {
          return;
        }
        checkbox.checked = !checkbox.checked;
        setBatchDeleteRowState(checkbox);
        syncBatchDeleteSelectAll();
      });
