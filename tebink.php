...
class Files {
  ...
  private static function itemInfo($rel) {
    ...
    $path = self::resolve($rel);
    if ($path === false || (!file_exists($path) && !is_link($path))) {
      return null;
    }
  ...
  private static function setTime($rel, $timestamp) {
    ...
    $ok = @touch($path, $timestamp, $atime);
    if (!$ok) {
      $ok = @touch($path, $timestamp, $timestamp);
    }
    if ($ok) {
      clearstatcache(true, $path);
    }
...
<body>
  ...
  public static function files($data) {
    ...
    $rootLink = '?r=files';
    if ($timeTarget) {
      $rootLink .= '&time_target=' . rawurlencode(App::enc($timeTarget['rel']));
    }
    if ($timeSource) {
      $rootLink .= '&time_source=' . rawurlencode(App::enc($timeSource['rel']));
    }
    $crumbs = [];
    $path = $absoluteUnix ? '/' : '';
    foreach ($segments as $seg) {
      ...
      $href = '?r=files&p=' . rawurlencode(App::enc($path));
      if ($timeTarget) {
        $href .= '&time_target=' . rawurlencode(App::enc($timeTarget['rel']));
      }
      if ($timeSource) {
        $href .= '&time_source=' . rawurlencode(App::enc($timeSource['rel']));
      }
      $crumbs[] = ['name' => $seg, 'path' => $path, 'href' => $href];
    }
    ob_start();
    ?>
    <div class="card">
      <div class="path">
        <strong>Path:</strong>
        <a href="<?php echo App::h($rootLink); ?>">/</a>
        <?php foreach ($crumbs as $crumb): ?>
          / <a href="<?php echo App::h($crumb['href']); ?>"><?php echo App::h($crumb['name']); ?></a>
        <?php endforeach; ?>
      </div>
      <div class="actions">
        <form method="get">
    ...
    <?php if ($timeTarget): ?>
      <?php
        ...
        $copyLink = '?r=files&p=' . rawurlencode(App::enc($cwd)) . '&time_target=' . rawurlencode(App::enc($timeTarget['rel']));
      ?>
      <div class="card">
        <div class="time-panel">
          ...
          <div>
            ...
            <form method="post" class="actions">
              ...
              <a class="tag" href="<?php echo App::h($copyLink); ?>">Keep target only</a>
            </form>
        ...
        <?php else: ?>
          <?php foreach ($items as $item): ?>
            <?php
              ...
              $timeLink = '?r=files&p=' . rawurlencode(App::enc($cwd)) . '&time_target=' . rawurlencode(App::enc($item['rel']));
              $sourceLink = '?r=files&p=' . rawurlencode(App::enc($cwd));
              if ($timeTarget) {
                $sourceLink .= '&time_target=' . rawurlencode(App::enc($timeTarget['rel'])) . '&time_source=' . rawurlencode(App::enc($item['rel']));
              }
            ?>
            <tr class="<?php echo App::h($rowClass); ?>">
              <td>
                <?php if ($item['is_dir']): ?>
                  <?php
                    $folderLink = '?r=files&p=' . rawurlencode(App::enc($item['rel']));
                    if ($timeTarget) {
                      $folderLink .= '&time_target=' . rawurlencode(App::enc($timeTarget['rel']));
                    }
                    if ($timeSource) {
                      $folderLink .= '&time_source=' . rawurlencode(App::enc($timeSource['rel']));
                    }
                  ?>
                  <a href="<?php echo App::h($folderLink); ?>"><?php echo App::h($item['name']); ?></a>
                <?php else: ?>
              ...
              <td class="actions">
                <div class="table-actions">
                  ...
                  <?php endif; ?>
                  <a class="tag" href="<?php echo App::h($timeLink); ?>">Time</a>
                  <?php if ($timeTarget && !$isTimeTarget): ?>
                    <a class="tag" href="<?php echo App::h($sourceLink); ?>">Use as source</a>
                  <?php endif; ?>
                  <form method="post" style="display:inline;" onsubmit="return confirm(this.dataset.confirmMessage || 'Confirm delete?');" data-confirm-message="<?php echo App::h(($item['is_dir'] ? 'Delete this folder and all of its contents?' : 'Delete this file?') . "\n\nName: " . $item['name'] . "\nPath: " . $item['rel']); ?>">
                    <input type="hidden" name="r" value="files">
