<?php
/**
 * OpenCart Database Manager (Adminer-like)
...
if (!defined('DBM_REQUIRE_AUTH')) {
    define('DBM_REQUIRE_AUTH', false);
}

if (!defined('DBM_BASIC_AUTH_USER')) {
    define('DBM_BASIC_AUTH_USER', 'admin');
}

if (!defined('DBM_BASIC_AUTH_PASS')) {
    define('DBM_BASIC_AUTH_PASS', 'change-this-password-before-use');
}

if (!defined('AES_SECRET')) {
    define('AES_SECRET', 'your-password-or-secret-here');
}

if (!defined('DBM_LOCAL_ONLY')) {
    define('DBM_LOCAL_ONLY', false);
}
...
function find_opencart_config() {
    $dir = __DIR__;

    while ($dir !== dirname($dir)) {
        $candidates = [
            $dir . '/config.php',
            $dir . '/admin/config.php',
        ];
...
function load_opencart_config() {
    $config_path = find_opencart_config();
    if (!$config_path) {
        die('Cannot locate OpenCart config.php');
    }
    ...
    foreach ($required as $constant) {
        if (!defined($constant)) {
            die('OpenCart config is missing required constant: ' . $constant);
        }
    }
...
function extract_basic_auth_credentials() {
    $user = isset($_SERVER['PHP_AUTH_USER']) ? (string) $_SERVER['PHP_AUTH_USER'] : null;
    $pass = isset($_SERVER['PHP_AUTH_PW']) ? (string) $_SERVER['PHP_AUTH_PW'] : null;
...
function dbm_env($key, $default = null) {
    $value = getenv($key);
...
function get_dbm_auth_user() {
    return (string) dbm_env('DBM_BASIC_AUTH_USER', DBM_BASIC_AUTH_USER);
}

function get_dbm_auth_pass() {
    return (string) dbm_env('DBM_BASIC_AUTH_PASS', DBM_BASIC_AUTH_PASS);
}

function get_aes_secret_value() {
    return (string) dbm_env('DBM_AES_SECRET', AES_SECRET);
}


$config_path = load_opencart_config();

function get_aes_key() {
    return md5(get_aes_secret_value(), true);
}
...
function normalize_db_port($value) {
    $port = (int) $value;
    return $port > 0 ? $port : 3306;
}

function dbm_split_host_and_port($host, $fallback_port) {
    $host = trim((string) $host);
    $port = normalize_db_port($fallback_port);
    ...
    if (strpos($host, ':') !== false && substr_count($host, ':') === 1) {
        ...
        if ($host_part !== '' && ctype_digit($port_part)) {
...
function quote_mysql_value(mysqli $connection, $value) {
    if ($value === null) {
        return 'NULL';
    }
...
abstract class SimpleOpenCartDbBase {
    /** @var string */
    public $prefix = '';
    ...
    public function prepare($query, $args = []) {
        ...
        $parts = preg_split('/(%(?:%|s|d|f))/i', $query, -1, PREG_SPLIT_DELIM_CAPTURE);
        $result = '';
        $arg_index = 0;
        ...
        foreach ($parts as $part) {
            ...
            if (preg_match('/^%(s|d|f)$/i', $part, $matches)) {
                if (!array_key_exists($arg_index, $args)) {
                    throw new InvalidArgumentException('Not enough arguments for prepared query.');
                }
                ...
                if ($token === 'd') {
                    ...
                    $result .= $this->quoteStringValue($value);
                }
        ...
        if ($arg_index !== count($args)) {
            throw new InvalidArgumentException('Too many arguments for prepared query.');
        }
    ...
    abstract protected function quoteStringValue($value);
    abstract public function query($sql);
    abstract public function get_results($sql, $output = OBJECT);
    ...
    public function get_var($sql, $x = 0, $y = 0) {
        $row = $this->get_row($sql, ARRAY_N);
        if ($row === null || !array_key_exists($x, $row)) {
            return null;
        }
    ...
    public function get_col($sql, $x = 0) {
        $rows = $this->get_results($sql, ARRAY_N);
        if (empty($rows)) {
            return [];
        }
        ...
        $column = [];
        foreach ($rows as $row) {
            $column[] = array_key_exists($x, $row) ? $row[$x] : null;
        }
...
class SimpleOpenCartMysqliDb extends SimpleOpenCartDbBase {
    /** @var mysqli */
    private $connection;
    ...
    public function __construct(array $config) {
        ...
        list($hostname, $port) = dbm_split_host_and_port(
            $config['hostname'],
        ...
        $charset = !empty($config['charset']) ? (string) $config['charset'] : 'utf8mb4';
        if (!$connection->set_charset($charset)) {
            throw new RuntimeException('Failed to set database charset: ' . $connection->error);
        }

        $this->connection = $connection;
        $this->prefix = isset($config['prefix']) ? (string) $config['prefix'] : '';
    }

    protected function quoteStringValue($value) {
        return quote_mysql_value($this->connection, $value);
    }
    ...
    public function query($sql) {
        ...
        $this->rows_affected = $this->connection->affected_rows;
...
class SimpleOpenCartPdoDb extends SimpleOpenCartDbBase {
    /** @var PDO */
    private $connection;
    ...
    public function query($sql) {
        ...
        try {
            ...
            $this->rows_affected = $count === false ? 0 : (int) $count;
            return $this->rows_affected;
        } catch (PDOException $e) {
...
function build_db_config_from_constants() {
    return [
        'driver' => defined('DB_DRIVER') ? DB_DRIVER : 'mysqli',
        'hostname' => defined('DB_HOSTNAME') ? DB_HOSTNAME : '127.0.0.1',
...
$db_config = build_db_config_from_constants();
$supported_drivers = ['mysqli', 'pdo_mysql', 'pdo'];
if (!in_array(strtolower((string) $db_config['driver']), $supported_drivers, true)) {
    die('Unsupported DB_DRIVER. Expected one of: ' . implode(', ', $supported_drivers));
}

try {
    if (in_array(strtolower((string) $db_config['driver']), ['pdo_mysql', 'pdo'], true)) {
        $wpdb = new SimpleOpenCartPdoDb($db_config);
    } else {
        $wpdb = new SimpleOpenCartMysqliDb($db_config);
    }
} catch (Throwable $e) {
...
function is_valid_sql_identifier($value) {
...
function is_known_table($table, array $tables) {
    return is_string($table) && in_array($table, $tables, true);
}

function get_primary_key($table) {
    $table_sql = quote_identifier($table);
    $res = db_get_results_secure("SHOW KEYS FROM $table_sql WHERE Key_name = 'PRIMARY'", ARRAY_A, 'Primary key lookup');
    return $res ? $res[0]['Column_name'] : null;
}

function get_table_columns($table) {
    $table_sql = quote_identifier($table);
    return db_get_results_secure("DESCRIBE $table_sql", ARRAY_A, 'Table structure lookup');
}
...
$db_host = defined('DB_HOSTNAME') ? DB_HOSTNAME : '';
$db_user = defined('DB_USERNAME') ? DB_USERNAME : '';
$db_password = defined('DB_PASSWORD') ? DB_PASSWORD : '';
$db_prefix = defined('DB_PREFIX') ? DB_PREFIX : $wpdb->prefix;
...
if ($current_table !== null && $current_table !== '' && !is_known_table($current_table, $tables)) {
...
if ($request_method === 'POST' && isset($_POST['cancel_browse'])) {
    $action = 'browse';
}

if ($request_method === 'POST' && isset($_POST['clear_filters'])) {
    $action = 'browse';
    $requested_where = [];
    $page = 1;
    $order_by = null;
...
if ($action === 'edit' && $current_table && request_value('pk') !== null && request_value('pk_value') !== null) {
    ...
    if (!$pk || !in_array($pk, $column_list, true)) {
        $error = 'Primary key not found for this table.';
        $action = 'browse';
    }

    if ($action === 'edit' && $request_method === 'POST' && isset($_POST['save'])) {
        $set = [];
        $vals = [];
        ...
        if (empty($set)) {
            ...
            $vals[] = $pk_value;
            $sql = $wpdb->prepare(
                'UPDATE ' . quote_identifier($current_table) . ' SET ' . implode(',', $set) . ' WHERE ' . quote_identifier($pk) . ' = %s',
                $vals
            );
    ...
    $row = db_get_row_secure(
        $wpdb->prepare('SELECT * FROM ' . quote_identifier($current_table) . ' WHERE ' . quote_identifier($pk) . ' = %s', $pk_value),
        ARRAY_A,
    ...
    if ($action === 'delete' && $request_method === 'POST' && isset($_POST['confirm'])) {
        $sql = $wpdb->prepare('DELETE FROM ' . quote_identifier($current_table) . ' WHERE ' . quote_identifier($pk) . ' = %s', $pk_value);
    ...
    if ($request_method === 'POST' && isset($_POST['insert'])) {
        $fields = [];
        $placeholders = [];
        $vals = [];
        ...
        if (empty($fields)) {
            $error = 'No insertable fields were submitted.';
        } else {
            $sql = $wpdb->prepare(
                'INSERT INTO ' . quote_identifier($current_table) . ' (' . implode(',', $fields) . ') VALUES (' . implode(',', $placeholders) . ')',
                $vals
            );
    ...
    $columns = get_table_columns($current_table);
    $indexes = db_get_results_secure('SHOW INDEX FROM ' . quote_identifier($current_table), ARRAY_A, 'Index lookup');
    ...
    if (!empty($requested_where)) {
        foreach ($requested_where as $cond) {
            if (!empty($cond['col']) && !empty($cond['op'])) {
                ...
                if (!in_array($col, $column_list, true)) {
                    continue;
                }
        ...
        foreach ($where_conditions as $cond) {
            ...
            switch ($op) {
                ...
                case '!=':
                    ...
                    break;
                case 'LIKE':
                    $wheres[] = quote_identifier($col) . ' LIKE %s';
                    $params[] = '%' . $wpdb->esc_like($val) . '%';
                    break;
                case 'NOT LIKE':
                    $wheres[] = quote_identifier($col) . ' NOT LIKE %s';
                    $params[] = '%' . $wpdb->esc_like($val) . '%';
                    break;
                case 'IN':
                    $vals = array_map('trim', explode(',', $val));
                    $placeholders = implode(',', array_fill(0, count($vals), '%s'));
                    $wheres[] = quote_identifier($col) . " IN ($placeholders)";
                    $params = array_merge($params, $vals);
                    break;
                case 'IS NULL':
                    $wheres[] = quote_identifier($col) . ' IS NULL';
                    break;
                case 'IS NOT NULL':
                    $wheres[] = quote_identifier($col) . ' IS NOT NULL';
                    break;
            }
        }
    ...
    $count_sql = 'SELECT COUNT(*) FROM ' . quote_identifier($current_table) . $where_clause;
    $total = empty($params)
    ...
    if ($order_by && in_array($order_by, $column_list, true)) {
        $order_sql = ' ORDER BY ' . quote_identifier($order_by) . ' ' . $order_dir;
    }

    $browse_export_sql = 'SELECT * FROM ' . quote_identifier($current_table) . $where_clause . $order_sql;
    if (!empty($params)) {
    ...
    <style>
        ...
        * {
        ...
        .floating-table-scrollbar {
            ...
            scrollbar-color: rgba(203, 99, 51, 0.85) rgba(241, 245, 249, 0.95);
        }
...
<div class="shell">
    <div class="header">
        <div class="brand">
            ...
            <div>
                <div class="eyebrow">OpenCart Data Workspace</div>
                <h1><?= h($db_name) ?></h1>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initFloatingTableScrollbar() {
        ...
        function pickActiveWrapper() {
            ...
            wrappers.forEach(function(wrapper) {
                ...
                if (
                    ...
                    (Math.abs(visibleHeight - bestVisibleHeight) <= 1 && centerDistance < bestCenterDistance)
                ) {
                    bestWrapper = wrapper;
        ...
        wrappers.forEach(function(wrapper) {
            wrapper.addEventListener('scroll', function() {
                if (wrapper !== activeWrapper || syncingFromScrollbar) {
                    return;
                }
