...
function compressWithPhpZip($root_path, $backup_dir_name, $backup_path, $patterns_csv, $exclude_patterns_csv, $max_bytes, $log_path, $fallback_note = '') {
    $log = array();
    $log[] = '[' . date('Y-m-d H:i:s') . '] PHP zip mode start';
    if ($fallback_note !== '') $log[] = $fallback_note;
...
function runCommandCapture($cmd, &$exit_code = null) {
    $disabled = getDisabledFunctions();
...
function isShellCommandAvailable($command) {
    ...
    $exit_code = null;
    $output = runCommandCapture('command -v ' . escapeshellarg($command) . ' 2>/dev/null', $exit_code);
    if ($output === null || $output === false) return false;

    $output = trim((string)$output);
    if ($output === '') return false;

    return ($exit_code === null || $exit_code === 0);
}

function inspectCompressionEngine() {
    $info = array(
        'use_shell' => false,
        'mode' => 'php',
        'engine' => 'php-zip',
    ...
    if (DIRECTORY_SEPARATOR === '\\') {
        $info['reason'] = 'windows_platform';
        return $info;
    }

    if (!canLaunchBackground()) {
        $info['reason'] = 'background_launcher_unavailable';
        return $info;
    }

    $required = array('sh', 'find', 'tar', 'gzip');
    foreach ($required as $command) {
        if (!isShellCommandAvailable($command)) $info['missing_commands'][] = $command;
    }

    if (!empty($info['missing_commands'])) {
        $info['reason'] = 'missing_commands';
        return $info;
    }
...
function buildPhpFallbackNote($compression_info) {
    if (!is_array($compression_info)) return '';

    if (!empty($compression_info['missing_commands'])) {
        return '未检测到系统命令 ' . implode(', ', $compression_info['missing_commands']) . '，已自动回退为 PHP ZipArchive 压缩。';
    }

    $reason = isset($compression_info['reason']) ? $compression_info['reason'] : '';
    if ($reason === 'background_launcher_unavailable') {
        return '命令执行函数不可用，已自动回退为 PHP ZipArchive 压缩。';
    }
    if ($reason === 'windows_platform') {
        return '当前环境不支持后台 shell 压缩，已自动回退为 PHP ZipArchive 压缩。';
    }
    return '';
}
...
if (isset($_GET['action']) && $_GET['action'] === 'backup') {
    ...
    $compression_info = inspectCompressionEngine();
    $use_shell_mode = !empty($compression_info['use_shell']);
    $php_fallback_note = $use_shell_mode ? '' : buildPhpFallbackNote($compression_info);
    ...
    writeStatus($status_file, array(
        ...
        'missing_commands' => isset($compression_info['missing_commands']) ? $compression_info['missing_commands'] : array(),
        'fallback_reason' => isset($compression_info['reason']) ? $compression_info['reason'] : '',
        'started_at' => date('Y-m-d H:i:s'),
        'pid' => null,
        'message' => '备份任务已启动。',
    ));

    if (!$use_shell_mode && $php_fallback_note !== '') {
        $st_bootstrap = readStatus($status_file);
        $st_bootstrap['message'] = $php_fallback_note;
        writeStatus($status_file, $st_bootstrap);
    }
    ...
    if ($use_shell_mode) {
        ...
        $ret = compressWithPhpZip($root_path, $backup_dir, $backup_path, $INCLUDE_PATTERNS, $EXCLUDE_PATTERNS, $max_bytes, $log_path, $php_fallback_note);
        ...
        if ($ok) {
            ...
            if ($php_fallback_note !== '') $st_now['message'] = $php_fallback_note . ' ' . $st_now['message'];
